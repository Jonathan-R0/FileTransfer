...
class InitialHandshakePackage:
    ...
    def __init__(self, data: bytes):
        self.is_upload, self.is_saw, self.file_size, self.file_name = struct.unpack('!??I255s', data)

    def __init__(self, is_upload: bool, is_saw: bool, file_size: int, file_name: str):
    ...
    def pack_data_send(self) -> bytes:
        return struct.pack(f'!??I255s', self.is_upload, self.is_saw, self.file_size, self.file_name)
    ...
    def pack_initial_handshake_return(self) -> bytes:
        ...
        return struct.pack(f'!??I255sII', self.is_upload, self.is_saw, self.file_size, self.file_name, 0, 0)
